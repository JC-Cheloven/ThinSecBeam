...
class Recta:
    ...
    def intersec(self,r2):
        ...
        if abs( self.es[0]*r2.es[0]+self.es[1]*r2.es[1] -1.) < 1.e-6:
            texto='// Intersect Error: lines do not intersect.\n'
            print(texto)
            quit() 
        
        det= -self.es[0]*r2.es[1]+ self.es[1]*r2.es[0]
        if abs(det)<1.0e-9: print('W: bad conditioned det in intersect.')
    ...
    for fila in filas_ptos:
        ...
        if x0 != '':
            try:
                ...
                texto= 'There is an error in the points. Please ckeck'
                texto+=' the completeness of your data etc.'
                texto+='\n             '+ str(sys.exc_info()[0])
                messagebox.showinfo(message=texto,title='BAD',parent=v9)
                return(1)
            p.y, p.z = y,z
            ptos[ip] = p
    ...
    for fila in filas_tramos:
        ...
        if x[0] != '':
            try:
                ...
                texto= 'There is an error in the tracts. Please check'
                texto+='whether a decimal point is needed or not, etc'
                texto+='\n             '+ str(sys.exc_info()[0])
                messagebox.showinfo(message=texto,title='BAD',parent=v9)
                return(1)
            ...
            if x[4] != '':
                try:
                    ...
                    texto= 'A radius R is not understood in the tracts.'
                    texto+='Please check decimal points, spaces etc.'
                    texto+='\n             '+ str(sys.exc_info()[0])
                    messagebox.showinfo(message=texto,title='BAD',parent=v9)
                    return(1)
...
def entraOpc():
    ...
    v4=Toplevel(v9, background='#D9D9D9')
    v4.title('Options')
    ...
    a= 'You can choose the point considered as motionless\n'
    a+='(ux=0) in calculations. The choice is indeed arbitrary.\n' 
    a+='This option requires re-calculating. Do that if you \n'
    a+='use it. Please consider using the other options\n'
    a+="whenever possible (they don't need re-calculating)."
    ...
    ttk.Label(v4, text='Point of zero ux displacement').grid(row=1,
        column=2, sticky='w')

    ttk.Separator(v4, orient=HORIZONTAL).grid(row=2,
        column=2,sticky='ew')

    a = '3D warping figures can be shown displaced by\n'
    a += 'an offset to get a clearer view. Options are:'
    ...
    ttk.Radiobutton(v4, variable=uT_di, value='0').grid(row=5,
        column=1)
    ttk.Label(v4, text='Draw as calculated').grid(row=5,
        column=2, sticky='w')
    ...
    ttk.Radiobutton(v4, variable=uT_di, value='1').grid(row=6,
        column=1)
    ttk.Label(v4, text='Draw outside the section').grid(row=6,
        column=2, sticky='w')
    ...
    ttk.Radiobutton(v4, variable=uT_di, value='2').grid(row=7,
        column=1)
    ttk.Label(v4,text='Draw average-centered').grid(row=7,
        column=2, sticky='w')
    ...
    a= 'Sometimes numerical labels in graphics are\n'
    a+='bothersome. Their plotting can be disabled here.\n'
    a+='The data checking window and the datasheet of the\n'
    a+='section are not affected by this option.'
    ttk.Label(v4, text=a, foreground='green').grid(row=9,
        column=2, sticky='w')
    ttk.Separator(v4, orient=VERTICAL).grid(row=9,
        column=0, sticky='nse')
    
    ttk.Label(v4,text='Labeling 2D graphics').grid(row=10,
        column=2, sticky='w')
    ttk.Checkbutton(v4, variable=etiq2D_di).grid(
        row=10, column=0, sticky='ns', columnspan=2)
    
    ttk.Label(v4,text='Labeling 3D graphics').grid(
    ...
    a= 'Details in warping 3D graphics are better\n'
    a+='perceived in a big scale plot, but they are\n'
    a+='more intuitive at sight at a smaller scale.'
    ttk.Label(v4, text=a, foreground='green').grid(row=13,
        column=2, sticky='w')
    
    ttk.Label(v4,text='Moderate warping plots').grid(row=14,
        column=2, sticky='w')
    ttk.Checkbutton(v4, variable=peque3D_di).grid(
        row=14, column=0, sticky='ns', columnspan=2)
    
    
    ttk.Button(v4, text='Done', command=hechoOpc).grid(
        row=15, column=0, columnspan=3)
...
def presenta_elige():
    ...
    def licencia():
        v1=Toplevel(v0)
        texto='''This program is Free Software licensed under the
GNU General Public License version 3.

To sum up, you can:
* Freely use the program
* Make copies and freely distribute them
* Read the code and learn how it works
* Make modifications/improvements to the program

Under the conditions below::
* If made public, modifications of the program must be
released under a license similar to this (thus software
can improve with other's contributions, as it is 
done in science).
* Modifications and derived works in general must
acknowledge the original author (you cannot present
yourself as the author of the program).
* In the last case, you should give credit to the
author as: Juan Carlos del Caño, assistant professor
at Escuela de Ingenierías Industriales, University of 
Valladolid (Spain)

This program is offered in the hope that it may be useful
but without any warranty. Please read the GNU General
...
Boston, MA 02110-1301, USA.
        ...
        ttk.Label(v1, text='Summary of license terms', 
    ...
    def breviario():
        ...
        v2=Toplevel(v0)
        texto='''- What is this program for:

It analyzes a thin-walled cross-section of a beam under linear 
behaviour. Such beams are typically metallic. Given the geometry
of the section, the following is calculated:
-- Barycenter, principal inertias and axes, section modulus, 
core of the cross-section, torsion stiffness (J), warping modulus
(Ia), shear centre (E), shear twirl tensor (see the manual), and 
shear areas.
-- If values are provided for one or more resultant stresses 
(shear forces, bending moments, axial load) several plots will be
drawn. They are high value for illustrative purposes and include 
practicable 3D graphics of the section warping (shear & torsion).
Text output offers additional info.

- How does it do it:

The thin-walled mathematical model for beams is assumed thoroughly
both for geometry and stress. It is up to the user to judge in the
first place whether this model is suitable regarding the particular 
problem or not. 
In the program, the geometry of the cross-section of the beam
consists of straight tracts and circular tracts (as usually is 
the case in the real world). Circular ones are often joint radiuses
but can also be "big" tracts from a modified circular pipe etc. 
Thus the geometry will be "exactly" modelled up to the thin-walled 
model of the section. Furthermore, displacements and stress fluxes
(shear and/or torsion) are also calculated in an "exact" manner by
using analytical expressions. Derived results as the torsion 
stiffness, warping modulus and shear centre are also "exactly"
calculated (thanks to the SymPy module that was used to obtain
the pertinent long expressions). The eventual presence or not of 
one or more closed paths in the section, in any configuration
or arrangement, is irrelevant to the program.

- Hints for correct usage:

 * The geometry of the cross-section is modelled using straight 
and/or circular TRACTs, each of constant thickness, each starting 
and ending in two POINTs of the section. POINTs are defined midway 
in the thickness of the TRACTs and are the connection points of 
the TRACTs. POINTs will have an identifying number and their y,z, 
coordinates are to be supplied. TRACTs will have also a number 
and their start and end points are to be supplied along with the 
TRACT thickness. If it is a circular TRACT, its radius and the 
sign of the angle are also to be supplied. Angles are positive 
counter-clockwise when going from the first to the second POINT. 
 * It makes usually no sense "to discretize" circular or straight 
portions of the section into TRACTs, as a single TRACT will achieve
the "exact" solution. The case of a continuous variation of the 
thickness in a branch of the section is pretty seldom found in the
real world, but it would be the one case where some "discretization"
would make sense (a TRACT has a constant thickness in the program). 
 * POINTs and TRACTs numbering can be freely assigned using positive
integer numbers. Numbering may not be compact or consecutive (may
not include all numbers lesser than the biggest one used).
 * In circular TRACTs, the arc is assumed to be the one with the 
shortest angular path. This greatly simplifies data input but
involves a limit of 180º for the arc. Shall your problem have a 
circular portion of more than 180º, simply use two TRACTs in the
modelling. 
 * Navigation: <Tab> & <Shift><Tab> to move among items in a 
window. <Space> hits the button you are on.

- Program development:

The present version of the program is 1.0. It includes all 
capabilities of version 0.8 (previous stable) plus the circular
tract. The user interface is now more flexible: modifying a problem
after being loaded or running several load cases is now allowed.
The data file of the program is still easily accessible for edition,
although you may never need that.

I hope you'll find ThinSecBeam useful.
        '''
        
        
        ttk.Label(v2, text='Quick notes', 
            font=('', 16)).grid(row=0, column=0, columnspan=2, pady=5)

        tcaja = Text(v2, width=55, height=30,wrap='word', font=('Sans',9),
            background='#D9D9D9', foreground='green', border=None, padx=20, pady=12)
        tcaja.grid(column=0, row=1, padx=8, sticky=(N,W,E,S))
        tcaja.insert('1.0',texto)
...
but also some obligations. Please read the license.
    '''
    ...
    texto='''If you don't know what this program is for or have 
a basic question about it, please read these quick notes. 
Or find further info in the manual.'''
    ...
    texto = '''You can start a problem from scratch. You'll specify 
geometry (points and tracts) and other general data.
You can also load a previously saved problem.'''
    ...
    texto = '''You can load a hardcoded example. It illustrates 
the basic capabilities of the program. It is also a quick way 
to check the program's installation and dependencies.'''
...
def salida_terminal():
    ...
    if esferico:
        print ('\nInertia tensor seems to be spherical. Considered as such.')
        print ('(all directions are principal with the same inertia).')

    if (not esferico):
        if yzsonPR :
            print ('\nyz axis seem to be principal. Considered as such.')


    print('\n  due to Vy=1:')
    print('ipt           ux            unbalance')
    ...
    print('\n\n###  Shear centre:  Ey, Ez  ###')
    print (E[0],' , ',E[1])
    print('in the given axis:')
    print(E[0]+Gvec[0], ' , ',  E[1]+Gvec[1])

    print('\n\nUnit torsion angle= ', theta_secc)
    print('Torsion modulus J = ', J_secc)

    print('\nDue to T=1:')
    print('pt             uT             unbalance')
    for ip, pto in ptos.items():
        a=0.
        for it in pto.inciP:
            tr=tramos[it]
    ...
    print('\nAverage of uT is: ', uT_medio)

    print('\nWarping modulus Ia= ', I_a)

    print('\n Shear Twirl Tensor C_ij: ')
    print(np.array(C_tensor))

    print ('\n\n .../ end of quick output in terminal /...\n\n')
    return()
...
def basico_tr():
    ...
    for it,tr in tramos.items():
        ...
        if tr.tipoT==0:
            ...
            alfa = signo*a
            if abs(alfa)<1.e-5:
                texto= 'The arc between points {:3d} and {:3d} is very '
                texto+='small (tract {:3d}). Please check the data.'
                texto+='\n             '+ str(sys.exc_info()[0])
...
def motor_calculo():
    ...
    if len(tramos)+1 > len(ptos):
        ...
        print('Number of points cannot be greater than tracts+1')
        print('Aborting.')
        quit()
    ...
    for itr,tr in tramos.items():
        ...
        '''
        print('tract ',itr,' with ',n,' interior tics.')
        s_tic=tr.L/(n+1.)
    ...
    print('\n\n actual n_tics for averaged C[] & uV = ',i)
    uVy_medio /= float(i)
    uVz_medio /= float(i)
...
def a_cargar_mas():
    ...
    texto ='''Solving problems with more tracts than provided by the user 
interface is made possible by this function. Please remember that 
straight & circular tracts are solved analytically (precission 
will not improve by using more of them). If you want to solve
a section by using many tracts anyway, please provide a hand-
tailored file with the data. The (simplified) user interface
will leave you at the point where you'd pressed the "calculate" 
button in a normal problem.
    '''
    a=messagebox.askokcancel(title='Question', message='Are you sure?',
...
def haz_informe():
    ...
    print('#####  ThinSecBeam 1.0 generated report #####', file=f)
    print('-'*60, file=f)
    
    print('\nIdentification: ', nfinforme, file=f)
    print('-'*60, file=f)
    print('\nPoints:', file=f)
    print('Base point= ', pto_base, file=f)
    print('       _____given axes_____    _____axes at G_____', file=f)
    ...
    print('\nTracts - basic parameters:', file=f)
    ...
    print('\nTracts - geometric parameters:', file=f)
    ...
    print('\nTracts - operation parameters:', file=f)
    print('{:>4s} {:>11s} {:>11s} {:>11s} {:>11s} {:>11s} '.format(
    ...
    if (  abs(C_tensor[0][0])+abs(C_tensor[1][1])<
        ...
        texto= '     Warning: shear areas seem to NOT make sense,\n'
        texto+='     at least in yz axis.'
        print(texto,file=f)
    else:
        print('Ok: Shear areas seem to make sense in yz axis.',file=f)
    ...
    print('\n{:>11s} {:>11s} {:^18s} {:^18s}'.format(
        'Wpsi','Weta','spherical I','principal yz'),file=f)
    print('{:> 11.5g} {:> 11.5g} {:^18s} {:^18s}'.format(
        Wpsi, Weta, a, b),file=f)
    print('     (W_psi is conditioned by yG,zG= {:10.4f} {:10.4f})'.format(
        ppsi[0], ppsi[1]), file=f)
    print('     (W_eta is conditioned by yG,zG= {:10.4f} {:10.4f})'.format(
    ...
    print('\nFor a torque Mx=1, and for the current base point, ', file=f)
    print('exact average of torsion displacement ux is:', file=f)
    print('            G*ux_mean = {:^14.7e}'.format(uT_medio), file=f)

    print('\nShear Twirl Tensor C_ij, with i,j in y,z:', file=f)
    print(np.array(C_tensor), file=f)
    
    print('\nIn inertia principal axis, C_psi,eta is:', file=f)
    a=np.array([v_psi,v_eta])
    C_psieta= np.matmul(np.matmul(a,C_tensor),np.transpose(a))
    print(np.array(C_psieta), file=f)
    
    print('\nIn self principal axis, C_diag is:', file=f)
    a=np.array([v_psi,v_eta])
    C_diag,v= np.linalg.eigh(C_tensor)
    print('C_I={:11.5g} ;  C_II={:11.5g}'.format(C_diag[0],C_diag[1]), file=f)
    print('with eigenvectors:', file=f)
    ...
    print('(angle from yz= {:6.3f} degrees)'.format(a), file=f)
    
    print('-'*60, file=f) 
    
    print('\nGuidance info on lateral buckling', file=f)
    print( '--- (not rigorous, please read the manual) ---\n',file=f)

    print('Bending on the strong axis (eta), and a typical steel ', file=f)
    print('for beams are assumed.\n', file=f)
    
    Llim= 16.02*np.sqrt(I_a/J_secc)
    texto='As a rough guidance:\nFor a beam of length = {:11.5g} or higher, \n'
    texto+='neglecting the effect of the warping modulus is reasonable.\n'
    print(texto.format(Llim),file=f)
    
    print('Guidance values of Mcr/E for some beam lengths,',file=f)
    print('being E~ 2.1e5 MPa (please translate to the problem units):',file=f)
    ...
    print('\nCore of the section: (y,z coordinates in G-centered axis; points', file=f)
    print('are ordered anti-clockwise):', file=f)
    print('    y          z', file=f)
    ...
    texto='The report has been saved in:\n'
    texto+= nfinforme
    messagebox.showinfo(message=texto, title='Confirmation', parent=v9)
    return()
...
def haz_ficha(recarga=True, para=True):
    ...
    ficha = plt.figure('Datasheet', figsize=(10., 6.9))
    ...
    texto  = 'Baricentre G (given axes):\n'
    texto += '  Gy={:10.5f}\n  Gz={:10.5f}\n\n'.format(Gvec[0],Gvec[1])
    texto += 'Inertias (axes by G):\n'
    texto += '  Iy={:9.4e}\n  Iz={:9.4e}\n  Iyz={:9.4e}\n\n'.format(
                                                                Iy,Iz,Iyz)

    texto += 'Principal inertias (axes by G):\n'
    texto += r'  I$\xi$'+'= {:9.4e}\n'.format(Ipsi)
    texto += r'  I$\eta$'+'= {:9.4e}\n'.format(Ieta)
    ...
    texto += 'Torsion stiffness '+'$J_T$={:10.5e}\n'.format(J_secc)
    texto += 'Warping modulus '+'$I_A$'+'={:10.5e}\n\n'.format(I_a)


    texto += 'Shear centre    (G-axes  O-axes):\n'  
    texto += '  Ey={:10.5f} ({:10.5f})\n'.format(E[0],E[0]+Gvec[0])
    texto += '  Ez={:10.5f} ({:10.5f})\n\n'.format(E[1],E[1]+Gvec[1])

    texto += 'Physical area= {:10.5f}\n'.format(A_secc)
...
def a_guardar():
    ...
    if bool(nfcompleto):
        ...
        print('\nN_ptos   N_tramos   base_point',file=f)
        ...
        print('###  End of provided data  ###', file=f)
        f.close()
        a='Data file has been saved in:\n{:}'.format(nfcompleto)
        messagebox.showinfo(message=a, title='Confirmatión',
             icon='info', parent=v9)
    f.close()
...
def a_salir():
    if messagebox.askokcancel(message='Do you want to close the program? ',
                detail='Unsaved data (if any) will be lost.', default='cancel',
                icon='question', title='Confirmation:',parent=v9) :
...
def haz_M(para=True):
    ...
    plt.figure('Bending stresses -2D')
    plt.title('{:}'.format(n_f))
    ax=plt.gca()    
    ...
    if trazar_LN:
        ...
        if LN.es[0] != 0.:
            ...
            # acota tr-c:
        plt.text(c[0], c[1], 'Tract', color='b', fontsize=11,
                horizontalalignment='center',verticalalignment='center',
        ...
        texto='Neutral line is far away.\nNot plotted.' 
    ...
    vent_sxx3D=plt.figure('Bending stresses -3D')
    ...
    if trazar_LN:
        ...
        if etiq3D:
            ...
            am=abs(sxx_tipo)/7
            ax.text(a[0], a[1], am*0.8, ' Trac', 'x',
                fontsize=11, color='b', fontweight='normal') 
            ax.text(a[0], a[1], -am, ' Comp', 'x', 
                fontsize=11, color='b', fontweight='normal') 
            am *= 0.8
...
def haz_T(para=True):
    ...
    vent_uV=plt.figure('Torsion warping')
...
def haz_V(para=True):
    ...
    for it,tr in tramos.items():
        ...
        if tr.tipoT == 0:
            ...
            print('unknown tract type: tr=',it,' tipo=',tipoT)
            quit()
    ...
    vent_uV=plt.figure('Shear warping')
    ax=vent_uV.gca(projection='3d')
    #ax.set_aspect('equal') sustituido por llamada a yz3D_iguales()
...
frame_ptos= ttk.Labelframe(fondoazul, text='Points',style='jc.TLabelframe',
    width=200,height=500)
frame_tramos= ttk.Labelframe(fondoazul, text='Tracts',style='jc.TLabelframe', 
    width=450,height=500)
frame_ptos.grid(column=0,row=0,rowspan=6,ipadx=3)
frame_tramos.grid(column=1,row=0,rowspan=6,ipadx=3)

frame_M= ttk.Labelframe(fondoazul, text='Bending',style='jc.TLabelframe', 
    width=170,height=170)
frame_M.grid(column=2,row=0,padx=3, pady=3, sticky='s')

frame_V= ttk.Labelframe(fondoazul, text='Shear',style='jc.TLabelframe', 
    width=170,height=130)
frame_V.grid(column=2,row=1,padx=3, pady=3)

frame_T= ttk.Labelframe(fondoazul, text='Torsion',style='jc.TLabelframe', 
    width=170,height=80)
...
boton_txt=ttk.Button(frame_botones,text='Report',width=8,
    command=haz_informe, state='disabled')
boton_txt.grid(column=0, row=0, sticky='sw', columnspan=2, padx=3, pady=2)

boton_cargar=ttk.Button(frame_botones, text='Load', width=6,command=a_cargar)
boton_cargar.grid(column=1, row=0, sticky='se', padx=3, pady=2)

boton_guardar=ttk.Button(frame_botones, text='Save',width=8,
    command=a_guardar, state='disabled')
boton_guardar.grid(column=0, row=1,sticky='sw', padx=3, pady=2)

boton_salir=ttk.Button(frame_botones, text='Quit',width=6, style='jc.TButton',
    command=a_salir).grid(column=1, row=1,sticky='se', padx=3, pady=3)

label_ptos1=ttk.Label(frame_ptos,text='pt ').grid(column=0, row=0, padx=3, pady=2)
...
label_tr3=ttk.Label(frame_tramos,text='end').grid(column=2, row=0, padx=3, pady=2)
